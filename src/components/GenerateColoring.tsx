...
export default function GenerateColoring() {
  ...
  const generate = async () => {
    ...
    try {
      ...
      if (data.error) {
        ...
        setError('Nepodařilo se vygenerovat obrázek. Zkus jinou postavu.');
      }
    } catch (err: any) {
  ...
  return (
    <div className="bg-robik-card rounded-2xl p-6 border border-pastel-blue/20">
      ...
      {/* Chyba */}
      {error && (
        <div className="bg-red-50 border border-red-200 rounded-xl p-3 text-sm text-red-600 mb-4 break-all">
          {error}
        </div>
      )}
      ...
      {imageUrl && (
        <div className="text-center">
          <img
            src={imageUrl}
            alt="Vygenerovaná omalovánka"
            className="max-w-full rounded-xl mx-auto mb-3 border border-gray-200"
          />
          <a
            href={imageUrl}
